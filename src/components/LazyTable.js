...
const LazyTable = ({ 
  ...
  const exportCSV = () => {
    if(exportCount === 0){
      ...
      axiosInstance.get(apiAll + "?with_data=true&filter=" + JSON.stringify(lazyCopy))
      .then((response) => {
  ...
  const onSelectAllChange = (event) => {
    ...
    if (selectAll) {
      ...
      if(apiAll){
        let lazyCopy = JSON.parse(JSON.stringify(lazyState));
        lazyCopy.custom_filters = dataPreparation(filterValues);  
        axiosInstance.get(apiAll + "?filter=" + JSON.stringify(lazyCopy))
        .then((response) => {
  ...
  <BlockUI blocked={blocked} className="flex items-start important-bg-white backdrop-blur-sm z-[0]" 
    ...
    }
    {
      ...
      <DataTable ref={dt} value={tableData} paginator={paginator} rows={(paginator ? (rowLimit && rowLimit || pageItem.value) : undefined)}
        ...
        pt={{root: {className: 'rounded-lg'}, loadingOverlay: {className: 'bg-opacity-10 rounded-lg'}}}
        scrollable={scrollable} scrollHeight={scrollHeight}
      >
        {checkbox && <Column headerClassName="bg-gray-100 rounded-tl-xl rounded-bl-xl border-none" selectionMode="multiple" headerStyle={{ width: '3rem' }} />}
        {
          columns.map((col, i) => {
        ...
        {actions && !customActions && <Column headerClassName="bg-gray-100 text-gray-500 text-xs tracking-wider uppercase rounded-tr-lg rounded-br-lg font-normal border-white border" header={'Actions'} headerStyle={{ width: '3rem' }} body={renderActions}/>}
        {actions && customActions && <Column headerClassName="bg-gray-100 text-gray-500 text-xs tracking-wider uppercase  rounded-tr-lg rounded-br-lg font-normal border-white border" header={'Actions'} headerStyle={{ width: customActionsWidth }} body={customActions}/>}
      </DataTable>
    }
